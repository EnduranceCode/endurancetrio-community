...
Tracker domain code lives in [`endurancetrio-tracker`](https://github.com/EnduranceCode/endurancetrio-tracker)
and is incorporated via `git merge` (see [`docs/tracker-integration.md`](docs/tracker-integration.md)).
The `tracker` remote is pre-configured.

The tracker domain is the Java packages under
`endurancetrio-*/src/main/java/com/endurancetrio/data/tracker/**` in ANY module (app, business,
data; main and test sources), plus any Flyway scripts under
`db/migration/{ddl,dml}/{h2,postgres}/` that target tracker-domain tables/entities (e.g. route,
telemetry-management). Everything else — including the majority of Flyway migrations and all DML
generated by the community `data-staging.sh` tooling (e.g. `V20260829.001` featured-events data) —
is community-owned.

Issues found in tracker-domain code/migrations go upstream; stage them as markdown in the tracker
repo's `issues/` using the tracker's issue template format. Issues in community-owned code,
migrations, or staging data are fixed directly here and are NOT staged upstream (this repo has no
`issues/` directory).
...
Profiles: `local` (localhost PG), `dev`/`prod` (env-var PG), `test` (H2). Activate via
`-Dspring.profiles.active=local` or `-Dspring-boot.run.profiles=local`.

...
/**
 * The {@link SportConverter} class is responsible for converting the {@link Sport} enum to its
 * corresponding database column representation and vice versa.
 * <p>
 * This converter is used to persist the {@link Sport} enum as a string in the database. It converts
 * the enum to its code when storing it in the database and converts the code back to the enum when
...
public class SportConverter implements AttributeConverter<Sport, String> {
  ...
  public Sport convertToEntityAttribute(String code) {
    return Stream.of(Sport.values())
        .filter(sport -> sport.getCode().equals(code.toUpperCase()))
        .findFirst()

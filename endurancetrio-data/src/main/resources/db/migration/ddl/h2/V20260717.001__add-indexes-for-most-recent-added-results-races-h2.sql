...
CREATE INDEX IF NOT EXISTS endurancetrio_hub.idx_individual_result_race_id_created_at
  ON endurancetrio_hub.individual_result (race_id, created_at DESC);

-- Index for findRacesWithMostRecentAddedResults: supports MAX(created_at) subquery on TeamResult
CREATE INDEX IF NOT EXISTS endurancetrio_hub.idx_team_result_race_id_created_at
  ON endurancetrio_hub.team_result (race_id, created_at DESC);

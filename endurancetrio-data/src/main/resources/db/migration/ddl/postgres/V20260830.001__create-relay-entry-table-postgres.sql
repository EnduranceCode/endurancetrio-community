...
CREATE TABLE IF NOT EXISTS relay_entry (
  id              BIGINT    NOT NULL,
  ...
  CONSTRAINT fk_relay_entry_team_id FOREIGN KEY (id) REFERENCES team(id) ON DELETE CASCADE
);

-- Create the relay_entry_athlete table
CREATE TABLE IF NOT EXISTS relay_entry_athlete (
  ...
  CONSTRAINT fk_relay_entry_athlete_relay_entry_id
    FOREIGN KEY (relay_entry_id) REFERENCES relay_entry(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_relay_entry_athlete_athlete_id ON relay_entry_athlete (athlete_id);

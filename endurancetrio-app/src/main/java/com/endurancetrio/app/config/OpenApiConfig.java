  ...
  public OpenAPI enduranceTrioOpenAPI() {
    return new OpenAPI().info(new Info().title("EnduranceTrio REST API").description("""
            **EnduranceTrio** is the definitive central hub for endurance sports data and resources,
            ...
            This REST API serves as the programmatic gateway to the **EnduranceTrio** platform,
            providing a secure and scalable interface for multisport event archives, results,
            and IoT telemetry integration. It is a specialized subset of the resources available
            on the main web platform.
            
            *Application Version: %s*""".formatted(appVersion)).version("v1.0.0"))

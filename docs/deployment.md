...
This document covers the deployment workflow for the **EnduranceTrio** project. For an overview
of the project, see the [main README.md](../README.md).
...
The **EnduranceTrio** application is optimized for containerized deployments using
...
```shell
sudo mkdir -p /opt/endurancetrio-community/logs
```
...
sudo chown -R endurancetrio:endurancetrio /opt/endurancetrio-community/logs/
```
...
```shell
ls -lag /opt/endurancetrio-community/logs/
```
...
| `ENDURANCETRIO_EXT_PORT` | The host port mapped to the API (e.g., `8080`)     | Yes      |
...
To complete deployment of **EnduranceTrio** application, execute the following command:

```shell
docker compose -p endurancetrio-community up -d
```

The output of the above command should show that **EnduranceTrio** application was deployed
with success. For a second confirmation, replace the placeholders in the below commands as
appropriate, and check its output.

```shell
docker ps
docker logs endurancetrio-community
curl -f http://localhost:{ENDURANCETRIO_EXT_PORT}/actuator/health
```

> **Placeholder Definition**
>
> + **{ENDURANCETRIO_EXT_PORT}** : The external port on the Docker host for accessing the application
...
To have a domain (or a subdomain) pointing to your **EnduranceTrio** application instance,
...
```shell
sudo wget -P /etc/apache2/sites-available/ https://raw.githubusercontent.com/EnduranceCode/endurancetrio-community/refs/heads/master/apache/vhost-template.conf
```
...
| <ENDURANCETRIO_EXT_PORT>  | External port where your  app runs (e.g., 8080, 8081)        |
